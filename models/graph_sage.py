...
class MLPSageLayer(nn.Module):
    """
    Encodes a node's using 'convolutional' GraphSage approach
    """
    def __init__(self, input_size, out_size, gcn=False): 
        super(MLPSageLayer, self).__init__()
    ...
    def forward(self, self_feats, aggregate_feats):
        ...
        combined = F.relu(self.model(combined))
        return combined
...
class GraphSage(nn.Module):
    ...
    def __init__(self, num_layers, input_size, out_size, raw_features, adj_lists, gcn=False, agg_func='MEAN'):
        ...
        for index in range(1, num_layers+1):
            ...
            setattr(self, 'sage_layer'+str(index), MLPSageLayer(layer_size, out_size, gcn=self.gcn))
    ...
    def forward(self, nodes_batch):
        ...
        # each nodes_batch_layers contains this layer's nodes and previous layers 
        # neighbor sampling and a mapping
    ...
    def aggregate(self, nodes, pre_hidden_embs, pre_neighs, num_sample=10):
        ...
        mask[row_indices, column_indices] = 1 # Each mask entry shows whether two nodes are neighbors
...
# Test this part of the code if necessary
class SageWrapper(nn.Module):
    def __init__(self, num_layers, input_size, out_size, raw_features, edges):
        super(SageWrapper, self).__init__()
        adj_list = self.build_adj_list(edges)
        self.sage_model = GraphSage(num_layers, input_size, out_size, raw_features, adj_list)

    def forward(self, idx):
        uniq_idx, reverse_idx = torch.unique(idx, return_inverse=True)
        out = self.sage_model(uniq_idx)
        emb = out[reverse_idx]
        return emb

    def build_adj_list(self, edge_list):
